...
import (
	dpfm_api_input_reader "data-platform-api-inspection-lot-doc-reads-rmq-kube/DPFM_API_Input_Reader"
	dpfm_api_output_formatter "data-platform-api-inspection-lot-doc-reads-rmq-kube/DPFM_API_Output_Formatter"
	"fmt"
...
func (c *DPFMAPICaller) HeaderDoc(
	...
	groupBy := "\nGROUP BY InspectionLot, DocType, DocIssuerBusinessPartner "

	rows, err := c.db.Query(
		`SELECT
    InspectionLot, DocType, MAX(DocVersionID), DocID, FileExtension, FileName, FilePath, DocIssuerBusinessPartner
		FROM DataPlatformMastersAndTransactionsMysqlKube.data_platform_inspection_lot_header_doc_data
		` + where + groupBy + `;`)
	if err != nil {
...
func (c *DPFMAPICaller) OperationDoc(
	...
	groupBy := "\nGROUP BY InspectionLot, Operations, OperationsItem, OperationID, DocType, DocIssuerBusinessPartner "

	rows, err := c.db.Query(
		`SELECT
    InspectionLot, Operations, OperationsItem, OperationID, DocType, MAX(DocVersionID), DocID, FileExtension, FileName, FilePath, DocIssuerBusinessPartner
		FROM DataPlatformMastersAndTransactionsMysqlKube.data_platform_inspection_lot_operation_doc_data
		` + where + groupBy + `;`)
	if err != nil {
		*errs = append(*errs, err)
		return nil
